...
func (self *liquidationPriceState) updateRoot(db Database) error {
	...
	// Legacy : root, _, err := self.trie.Commit(func(leaf []byte, parent common.Hash) error {
	root, _, err := self.trie.Commit(func(path []byte, leaf []byte, parent common.Hash) error {
