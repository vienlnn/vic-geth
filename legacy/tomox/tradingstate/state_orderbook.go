...
func (s *tradingExchanges) empty() bool {
	...
	if !params.EmptyHash(s.data.AskRoot) {
		return false
	}
	if !params.EmptyHash(s.data.BidRoot) {
		return false
	}
	if !params.EmptyHash(s.data.OrderRoot) {
		return false
	}
	if !params.EmptyHash(s.data.LiquidationPriceRoot) {
		return false
	}
	return true
}
...
func (self *tradingExchanges) CommitAsksTrie(db Database) error {
	...
	root, _, err := self.asksTrie.Commit(func(path []byte, leaf []byte, parent common.Hash) error {
...
func (self *tradingExchanges) CommitBidsTrie(db Database) error {
	...
	root, _, err := self.bidsTrie.Commit(func(path []byte, leaf []byte, parent common.Hash) error {
		var orderList orderList
		if err := rlp.DecodeBytes(leaf, &orderList); err != nil {
			return nil
		}
...
func (self *tradingExchanges) CommitLiquidationPriceTrie(db Database) error {
	...
	root, _, err := self.liquidationPriceTrie.Commit(func(path []byte, leaf []byte, parent common.Hash) error {
		var orderList orderList
		if err := rlp.DecodeBytes(leaf, &orderList); err != nil {
			return nil
		}
